...
test('reduce([1, 2], (acc, item) => { acc["number-" + item] = item; return acc }, {}) should return {"number-1": 1, "number-2": 2}', () => {
...
test('reduce([1, 2], (acc, item, index) => acc + index, 0) should return 1', () => {
  const before = reduce([1, 2], (acc, item, index) => acc + index, 0)
  const after = 1
  expect(before).to.be.equal(after)
})

test('reduce([1, 2], (acc, item, index, array) => acc + array[index], 0)', () => {
  const before = reduce([1, 2], (acc, item, index, array) => acc + array[index])
  const after = 3
  expect(before).to.be.equal(after)
})

test('reduceRight(["tor", "Vi"], (acc, item) => acc + item, "") should return "Vitor"', () => {
  const before = reduceRight(['tor', 'Vi'], (acc, item) => acc + item, '')
  const after = 'Vitor'
  expect(before).to.be.equal(after)
})
